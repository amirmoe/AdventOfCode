...
namespace AdventOfCode.Solutions.Year2021.Day07
{
    internal class Day07 : ASolution
    {
        ...
        private static int PartOneFuelCost(int crab, int destination) => Math.Abs(crab - destination);
        private static int PartTwoFuelCost(int crab, int destination) => Enumerable.Range(1, Math.Abs(crab - destination)).Sum();
    }
}